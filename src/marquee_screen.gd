...
const DESIGN_SIZE := Vector2i(1920, 360)

const LiftStatusPanelScene := preload("res://src/lift_status.gd")

@export var screen_index: int = 0
@export var background_color: Color = Color("3b2a04")
@export var accent_color: Color = Color("ffc84a")
...
func _ready() -> void:
	...
	title.offset_top = 65.0
	title.offset_bottom = title.offset_top + 120.0
	add_child(title)

	var identifier := Label.new()
	identifier.text = "MARQUEE"
	...
	lift_panel.compact = true
	add_child(lift_panel)

	add_child(_build_resolution_border(4))
...
func _process(delta: float) -> void:
	...
	if is_instance_valid(_bar):
		...
		_bar.position.x = (1.0 - progress) * (_bar_width - _bar.size.x)
...
func _build_diagnostics() -> Label:
	...
	diagnostics.position = Vector2(42, DESIGN_SIZE.y - 40)
...
func _build_moving_bar() -> void:
	...
	_bar.size = Vector2(360, 18)
	_bar.position.y = DESIGN_SIZE.y * 0.69
